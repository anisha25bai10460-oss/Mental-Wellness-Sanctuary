...
Category
Primary Technology / Tool
Role in the Project
I. Frontend & UI
HTML5, JavaScript (ES6+), Tailwind CSS
Provides the structure, logic, responsiveness, and calming, thematic aesthetic for the final web application interface.
II. Database & Cloud
Google Firestore
The primary NoSQL database used for real-time, multi-user data persistence (CRUD) and synchronization in the web environment.
III. Visualization
Chart.js (JavaScript)
Specialized library used to dynamically generate the Mood vs. Sleep line chart and other data visualizations in the browser.
IV. Security & Auth
Firebase SDKs
Handles user authentication and ensures all logged data is securely isolated per user ID within the cloud database.
V. ML & Advanced Logic (Python)
Python 3
The core language for the initial modular design and the underlying logic for advanced features (e.g., the initial design for prediction, reporting, and database management).
VI. Data Science (Python)
Scikit-learn & NumPy
Libraries used for the theoretical implementation of the Linear Regression model in the Python CLI project to predict next-day mood (the mock logic was transferred to JavaScript for the web version).
VII. CLI Persistence
SQLite & bcrypt
Used in the initial modular Python design to handle local file-based data storage (SQLite) and secure password hashing (bcrypt).
VIII. Supporting Tools
Font Awesome
Provides high-quality, scalable icons for visual enhancements throughout the web interface.
